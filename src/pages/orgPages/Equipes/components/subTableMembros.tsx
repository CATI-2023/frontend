...
import { equipe, membroEquipe } from "../../../../Types/type";
...
const ExpandableTableRow = ({ children, equipe, ...otherProps }: Props) => {
  ...
  async function updateMembroLider(membro: membroEquipe) {
    const data_ = {
    ...
    putMembroEquipe(membro.membro_equipe_id, data_)
      .then(() => {
        showNotification({
          type: "success",
          title: "Sucesso ao atualizar a Membro na Equipe.",
      ...
      .catch((err) => {
        showNotification({
          type: "error",
          title: "Erro ao atualizar Membro na Equipe.",
          message: "Membro não atualizado. " + err?.response?.data?.message,
        });
      });
  }
                        ...
                        ? equipe.MembroEquipe.map((membro: membroEquipe) => (
                            <TableRow key={membro.membro_equipe_id}>
                              ...
                              <TableCell align="center">
                                <IconButton
                                  color="primary"
                                  disabled={membro.lider}
                                  onClick={() => {
                                    updateMembroLider(membro);
                                  }}
                                >
                                  <DefaultsIcons.EventoVigenteIcon />
                                </IconButton>
                              </TableCell>
                              <TableCell align="center">
                                <IconButton
